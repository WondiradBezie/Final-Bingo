...
async function loadGames() {
    try {
        ...
        if (games.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" style="text-align: center;">No games found</td></tr>';
            return;
        }
        ...
        games.forEach(game => {
            ...
            row.innerHTML = `
                <td>${game.game_id.substring(0, 8)}...</td>
                <td>${game.room}</td>
...
async function loadUsers() {
    try {
        ...
        document.getElementById('user-active-today').textContent = data.activeToday;
        document.getElementById('user-new-today').textContent = data.newToday;
        document.getElementById('user-total-balance').textContent = formatCurrency(data.totalBalance);
        ...
        data.list.forEach(user => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    <button class="action-btn edit" onclick="adjustUserBalance('${user.id}', ${user.balance})">
...
async function loadTransactions() {
    try {
        ...
        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center;">Transaction history coming soon</td></tr>';
...
function loadRoomsSettings(rooms) {
    ...
    rooms.forEach(room => {
        ...
        roomDiv.innerHTML = `
            <div class="room-header" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                <input type="text" value="${room.name}" placeholder="Room Name" style="flex: 1; padding: 5px;">
                <button class="btn-icon" onclick="removeRoom('${room.id}')" style="margin-left: 10px;">
                    <i class="fas fa-trash"></i>
                </button>
            </div>
            <div class="room-details" style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
...
async function loadAuditLogs() {
    try {
        ...
        if (logs.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #666;">No logs found</p>';
            return;
        }
...
async function viewUser(userId) {
    try {
        ...
        content.innerHTML = `
            <div class="user-detail" style="margin-bottom: 10px;">
            ...
            <div class="user-detail" style="margin-bottom: 10px;">
                <strong>Status:</strong> ${user.is_banned ? 'Banned' : 'Active'}
            </div>
            <div class="user-detail" style="margin-bottom: 10px;">
                <strong>VIP:</strong> ${user.is_vip ? 'Yes' : 'No'}
            </div>
        `;
...
function adjustUserBalance(userId, currentBalance) {
...
async function processBalanceAdjustment() {
    ...
    try {
        ...
        if (result.success) {
            ...
            alert(result.message || 'Failed to adjust balance');
        }
...
function viewGame(gameId) {
    window.open(`/bingo_game.html?game=${gameId}&admin=true`, '_blank');
}
...
function exportUsers() {
    window.location.href = '/api/admin/export/users?token=' + getAuthToken();
}

function exportTransactions() {
    const from = document.getElementById('tx-date-from')?.value || '';
    const to = document.getElementById('tx-date-to')?.value || '';
    window.location.href = `/api/admin/export/transactions?from=${from}&to=${to}&token=${getAuthToken()}`;
}
...
function loadRooms() {
    ...
    fetch('/api/admin/rooms', {
        ...
        .catch(err => {
            ...
            container.innerHTML = '<p class="error">Error loading rooms</p>';
        });
}
...
function editRoom(roomId) {
    alert(`Edit room ${roomId} coming soon`);
}

function removeRoom(roomId) {
    if (confirm(`Are you sure you want to remove room ${roomId}?`)) {
        alert(`Remove room ${roomId} coming soon`);
    }
}

function resetSettings() {
    if (confirm('Reset all settings to default?')) {
        document.getElementById('setting-card-price').value = 10;
        document.getElementById('setting-prize-percent').value = 80;
        document.getElementById('setting-min-players').value = 2;
        document.getElementById('setting-max-players').value = 400;
        document.getElementById('setting-call-interval').value = 2;
        document.getElementById('setting-selection-time').value = 20;
...
function updateRevenueChart() {
    // This would update with new period data
    loadDashboardData();
}
...
// Adjust balance function for modal
function adjustBalance() {
    if (!currentUser) return;
    adjustUserBalance(currentUser.id, currentUser.balance);
}
